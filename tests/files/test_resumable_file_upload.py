...
class TestResumableUpload:
    ...
    def test_resumable_upload_full_at_once(
        self, co_auth_token, jpeg_file_path, jpeg_file_binary_data, root_dir_id
    ):
        ...
        with allure.step("Sending info about file to the server"):
            response = CoClient.post_upload_resumable_v1(
        ...
        with allure.step("Uploading full file"):
            upload_response = CoClient.patch_upload_resumable_v1(
        ...
        with allure.step("Getting list of files, checking presence of uploaded one"):
            all_files_data_response = CoClient.get_files()
    ...
    def test_resumable_file_upload_partial(
        ...
        with allure.step("Sending info about file to the server"):
            file_data_response = CoClient.post_upload_resumable_v1(
                upload_length=file_length,
                name=filename,
                type=Mediatype.jpeg,
                filetype=Mediatype.jpeg,
        ...
        with allure.step("Getting Upload Offset"):
            middle_of_upload_response = CoClient.head_upload_resumable_v1(path=path)
    ...
    def test_resumable_file_upload_stop(
        ...
        with allure.step("Stopping upload, deleting upload data"):
            CoClient.delete_upload_resumable_v1(path=path)
            CoClient.head_upload_resumable_v1(path=path, status_code=404)
            CoClient.patch_upload_resumable_v1(
                path=path,
                status_code=404,
                data=file_data_second_part,
                upload_offset=str(first_part_of_file_length),
